"""
Django settings for the Shared Expenses app.

Local dev uses SQLite. Production (Render) uses Postgres via DATABASE_URL.
...
SECRET_KEY = os.getenv(
    "SECRET_KEY",
    "django-insecure-dev-only-key-change-in-prod-kj3m6o3jfphg2ukodcsj7w9",
)

DEBUG = env_bool("DEBUG", True)

# Comma-separated in env; sensible dev defaults.
ALLOWED_HOSTS = [
    h.strip()
...
# Database: DATABASE_URL if present (Postgres in prod), else SQLite locally.
DATABASES = {
...
# LLM feature (explain-my-balance). Optional; degrades gracefully if absent.
